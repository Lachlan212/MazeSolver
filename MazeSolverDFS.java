 ...
 * MazeSolverDFS.java
 * Implements a DFS solution to a maze. It also utilises knowledge of the finish location in determining which path to take.
 * Stacks are used for constant time efficiency. 2d arrays are used similarly and accessed by index.
 */
...
public class MazeSolverDFS {
    ...
    public static void main(String[] args){
        ...
        String mazedata = MazeGenerator.readFromFile(filename);
        ...
        int[][] visited = new int[maxRow][maxColumn]; // initialises to 0 so no for loop needed
    ...
    private static boolean DFS(Stack<Integer> stack, int[] currentNode, int[][] visited) {
        ...
        // variable was removed from being stored to save memory
        ...
        if (cell_openness_list[getNumFromNode(currentNode)-1] == 3 || cell_openness_list[getNumFromNode(currentNode)-1] == 2){ // check down open
        ...
        if (cell_openness_list[getNumFromNode(currentNode)-1] == 3 || cell_openness_list[getNumFromNode(currentNode)-1] == 1) { // check right
        ...
        if(availableNeighbours.isEmpty()){ // No available neighbours
            return false;
        }
        ...
        for (int i = 0; i < availableNeighbours.size()-1; i++){
            ...
            // variables removed to save memory, instead recalculated each time it is needed.
            ...
            // swap nodes
            // if ( nextDistance < currentDistance ){ = next line
            if ( Math.sqrt(Math.pow( (availableNeighbours.get(i+1)[0] + finishNode[0]), 2) + Math.pow( (availableNeighbours.get(i+1)[1] + finishNode[1]), 2 ) ) < Math.sqrt(Math.pow( (availableNeighbours.get(i)[0] + finishNode[0]), 2) + Math.pow( (availableNeighbours.get(i)[1] + finishNode[1]), 2 ) ) ){
                Collections.swap(availableNeighbours, i, i+1);

                // reverse loop
                // note nextDistance is stored and can be used to compare against previous values
                for (int j=i; j>1; j++){
                    ...
                    if ( Math.sqrt(Math.pow( (availableNeighbours.get(i+1)[0] + finishNode[0]), 2) + Math.pow( (availableNeighbours.get(i+1)[1] + finishNode[1]), 2 ) ) < Math.sqrt(Math.pow( (availableNeighbours.get(j-1)[0] + finishNode[0]), 2) + Math.pow( (availableNeighbours.get(j-1)[1] + finishNode[1]), 2 ) )){
        ...
        for ( int i =0; i < availableNeighbours.size(); i++ ){
            boolean cellOnPath = DFS(stack, availableNeighbours.get(i), visited); // true if cell is on the way to finishing node else false
            if (cellOnPath){ // save result
                stack.push(getNumFromNode(currentNode));
                path = stack;
                return true;
            }
